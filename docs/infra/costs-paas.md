...
| Registry | Docker Hub para imagens Docker |
...
| Docker Hub | Imagens publicas do projeto | Plano gratuito / custo fora da AWS | USD 0.00 |
...
| **Total estimado AWS** |  |  | **USD 176.89/mes** |
...
O codigo atual publica e referencia imagens no Docker Hub, por exemplo `humbertosandmann/account:latest`, `humbertosandmann/auth:latest`, `humbertosandmann/product:latest` e `humbertosandmann/gateway:latest`. Portanto, esta estimativa nao considera custo de Amazon ECR. Caso o projeto migre as imagens para ECR, os manifestos Kubernetes, `compose.prod.yaml`, Jenkinsfiles e credenciais do pipeline devem ser atualizados em conjunto.
...
| Remover tags antigas no Docker Hub | Reduz acumulacao de imagens e facilita auditoria |
...
| Docker Hub | Registro de imagens Docker | Armazenamento, disponibilidade e distribuicao das imagens |
...
6. Registry de containers: o codigo atual usa Docker Hub; incluir ECR somente se as imagens forem migradas para AWS.
